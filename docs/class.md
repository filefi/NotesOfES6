...
```javascript
class Point {
  constructor() {   // 就连constructor都是定义在构造函数的prototype上的
    // ...
  }
...
**解决方法1：在构造方法中绑定`this`，这样就不会找不到`print`方法了：**
...
**解决方法3：使用`Proxy`，获取方法的时候，自动绑定`this`。**
...
上面的代码，一眼就能看出，`Person`类有两个实例属性，一目了然。另外，写起来也比较简洁。

**在类中定义实例属性不能使用`let`、`const`、`var`关键字，否则会报错：**

```js
class Person {
  let name = 'Mike';  // Uncaught SyntaxError: Unexpected identifier
  let age = 12;
...
class Person {
  const name = 'Mike';  // Uncaught SyntaxError: Unexpected identifier
  const age = 12;
...
class Person {
  var name = 'Mike';  // Uncaught SyntaxError: Unexpected identifier
  var age = 12;
...
**即使是声明未赋值的实例属性，也不用使用关键字`let`、`const`、`var`，否则也会报错：**

```js
class Person {
  let name; // Uncaught SyntaxError: Unexpected identifier
  let age;
...
let p2 = new Person()
p2.toString(); // undefined undefined
```
