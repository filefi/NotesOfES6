...
- 第1个参数表示拷贝开始的字节序号（含该字节）；
- 第2个参数表示拷贝截止的字节序号（不含该字节）。如果省略第二个参数，则默认到原`ArrayBuffer`对象的结尾。

`slice`方法其实包含2步：

- 第1步是先分配一段新内存
- 第2步是将原来那个`ArrayBuffer`对象拷贝过去。
...
- **`TypedArray`视图**，其数组成员都是同一个数据类型。
- **`DataView`视图**，其数组成员可以是不同的数据类型。
...
**这 9 个构造函数生成的数组，统称为`TypedArray`视图。**它们很像普通数组，都有`length`属性，都能用方括号运算符（`[]`）获取单个元素，所有数组的方法，在它们上面都能使用。
