...
如果脚本体积很大，下载和执行的时间就会很长，因此造成浏览器堵塞，用户会感觉到浏览器“卡死”了，没有任何响应。这显然是很不好的体验，所以浏览器允许脚本异步加载，**下面就是2种异步加载的语法：**
...
JavaScript 现在有2种模块。一种是 ES6 模块，简称 ESM；另一种是 CommonJS 模块，简称 CJS。
...
**一旦指定`type`字段为`module`，该目录里面的 JS 脚本，就被解释用 ES6 模块。**
...
console.log('a.mjs');
console.log(bar());
function foo() { return 'foo' }   // 函数具有提升效果，不会报错
export {foo};
...
console.log('b.mjs');
console.log(foo());
function bar() { return 'bar' }  // 函数具有提升效果，不会报错
export {bar};
```
...
console.log('a.mjs');
console.log(bar());
const foo = () => 'foo';   // 改成表达式，不具有提升效果，执行就会报错
export {foo};
```
