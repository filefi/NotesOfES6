...
- **第1个参数是`resolved`状态的回调函数；**

- **第2个参数（可选）是`rejected`状态的回调函数。**

**`then`方法返回：**

- **`then`方法返回的是一个新的`Promise`实例（注意，不是原来那个`Promise`实例）。因此可以采用链式写法，即`then`方法后面再调用另一个`then`方法。采用链式调用时，前一个回调函数完成以后，会将返回结果作为参数，传入后一个回调函数。**
...
**只有等到 *所有这些参数实例都返回结果*，不管是`fulfilled`还是`rejected`，包装实例才会结束。**该方法由 [ES2020](https://github.com/tc39/proposal-promise-allSettled) 引入。
...
上面代码中，`Promise.allSettled()`的返回值`allSettledPromise`，状态只可能变成`fulfilled`。它的监听函数接收到的参数是数组`results`。该数组的每个成员都是一个对象，对应传入`Promise.allSettled()`的2个 Promise 实例。**每个对象都有`status`属性，该属性的值只可能是字符串`fulfilled`或字符串`rejected`。`fulfilled`时，对象有`value`属性，`rejected`时有`reason`属性，对应2种状态的返回值。**
