...
**第1种情况，`super`作为函数调用时，代表父类的构造函数。ES6 要求，子类的构造函数必须执行一次`super`函数。**
...
这里需要**注意，由于`super`指向父类的原型对象，所以定义在父类实例上的方法或属性，是无法通过`super`调用的。**
...
**大多数浏览器的 ES5 实现之中，每一个对象都有`__proto__`属性，指向对应的构造函数的`prototype`属性。Class 作为构造函数的语法糖，同时有`prototype`属性和`__proto__`属性，因此同时存在2条继承链：**
...
***子类实例* 的`__proto__`属性的`__proto__`属性，指向 *父类实例* 的`__proto__`属性。也就是说，子类的原型的原型，是父类的原型。**
...
上面代码中，`ColorPoint`继承了`Point`，导致`ColorPoint`原型的原型是`Point`的原型。
...
**在ES6以前，这些原生构造函数是无法继承的，比如，不能自己定义一个`Array`的子类。**
